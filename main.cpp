...
int main()
{
	...
	string BSDL_dir = "C:/Users/changeneversay/Desktop/BSDL/BS-file/bsdl_file/ARRIA_10_10AS016CU19.bsdl";//若execute = 0,需要修改路径保证程序正确&&&&&
    ...
    {
		string database1 = BSDL_name +  "_BSDL_DATA";//获取数据库名字
		ifstream inFile_Netlist(dir);//获取网表文件流
		ostringstream temp_Netlist;
		temp_Netlist << inFile_Netlist.rdbuf();
		string Netlist_FileContent = temp_Netlist.str();//读取网表的内容
        m.Process_Fault_injection(password, dir, Netlist_name,database1, Netlist_FileContent, Net);//执行故障注入
		//system("pause");
		return 0;
	}
